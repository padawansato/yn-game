# yn-game
## 選択肢提示
ユーザーに選択肢を提示するときは、ユーザーが数字もしくはy/nで返答できるように聞きなさい。
質問するときはAskUserQuestionを使っても良い。
...
`.vue` / `.ts` ファイルが肥大化するとリファクタが重くなり、機能追加のハードルが上がる。以下を目安に責務分離を検討する:

- **500行**: リファクタを検討する水準。責務が複数混在していないか点検する
- **1000行**: 赤信号。機能追加時は先に責務抽出してから本題の実装に入る
- 機械的ルールではなく兆候。単一責務で必然的に長くなっているなら、それ以上でも許容

教訓: `src/App.vue` が 1149行に肥大化し、描画・ゲームロジック・シナリオ定義・状態管理が混在していた。Issue #47（ステージサイズ拡大）の実装時にこれが障害となった。

### 単一ソース・オブ・トゥルース (SSoT)

定数・設定値は **単一箇所** に定義し、全ての利用箇所からそれを参照する。同じ値を複数のファイルにハードコードしない。詳細は `.claude/rules/single-source-of-truth.md` を参照。
