...
- **WHEN** a Gajigajimushi in 'adult' phase has carryingNutrient >= GAJI_REPRO_NUTRIENT_COST AND life >= GAJI_REPRO_LIFE_COST
- **THEN** it SHALL produce 1 larva at an adjacent empty cell

#### Scenario: Reproduction cost
- **WHEN** a Gajigajimushi reproduces
- **THEN** the parent's carryingNutrient SHALL decrease by GAJI_REPRO_NUTRIENT_COST AND life SHALL decrease by GAJI_REPRO_LIFE_COST
